/**
 * Created by Alberto Soto Fernandez & Llorenç
 *
 * Requires:
 *    Nothing. No dependency declared to toastr or others
 *
...
(function ($) {
     ...
     *    };
        ...
        openModal: function  (title, content, buttonLabel){
            jQuery(this.getModalString(title, content, buttonLabel)).modal();  
        },
        getModalString: function (title,content,buttonLabel) {
            return this.format('<div class="modal fade" role="dialog"><div class="modal-dialog modal-sm"><div class="modal-content"><div class="modal-header"><h4 class="modal-title">{title}</h4></div><div class="modal-inner">{proceed}</div><div class="modal-footer"><button type="button" class="btn sbold" data-dismiss="modal" data-action = "cancelDeletion">{cancel}</button></div></div></div></div></div>',
                {
                    title: title,
                    proceed: content,
                    cancel: buttonLabel
        ...
        doAjax: function (url, method, sendData, fOnSuccessCallback, fOnErrorCallback) {
            ...
            var showMessage = $.backofficeApp.global.showMessage.bind();
